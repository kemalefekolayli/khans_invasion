...
public class Builder : MonoBehaviour
{
    ...
    public bool CanBuild(ProvinceModel province, string buildingType, float availableGold)
    {
        ...
        {
            if (logBuilding)
                GameLog.Log(GameLogCategory.Province, $"Building {buildingType} already exists in {province.provinceName}");
            return false;
        }
        ...
        {
            if (logBuilding)
                GameLog.Log(GameLogCategory.Province, $"Not enough gold for {buildingType}. Need {cost}, have {availableGold}");
            return false;
        }
    ...
    public float GetBuildingCost(string buildingType)
    {
        ...
        switch (buildingType)
        {
            ...
            default:
                GameLog.Warning(GameLogCategory.Province, $"Unknown building type: {buildingType}");
                return 9999f;
        }
    }
    ...
    public float BuildBuilding(ProvinceModel province, string buildingType, float availableGold)
    {
        ...
        if (logBuilding)
        {
            GameLog.Log(GameLogCategory.Province, $"Built {buildingType} in {province.provinceName} for {cost} gold");
        }
    ...
    private void ApplyBuildingBenefits(ProvinceModel province, string buildingType)
    {
        ...
        {
            case "Fortress":
                // TODO: Add fortress logic here (e.g. siege defense bonus)
                break;
