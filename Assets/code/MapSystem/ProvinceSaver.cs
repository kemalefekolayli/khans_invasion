...
public class ProvinceSaver : EditorWindow
{
    ...
    void SaveAllProvincesWithSprites()
    {
        ...
        if (mapGen == null)
        {
            GameLog.Error(GameLogCategory.Core, "MapGenerator bulunamadı!");
            return;
        }
        ...
        for (int i = 0; i < provinces.Length; i++)
        {
            ...
            {
                GameLog.Warning(GameLogCategory.Core, $"{province.provinceName} sprite'ı yok, atlanıyor...");
                continue;
            }
        ...
        GameLog.Log(GameLogCategory.Core, $"✓ {savedCount} province (texture + sprite + prefab) kaydedildi!");
    ...
    void LoadAllProvinces()
    {
        ...
        {
            GameLog.Error(GameLogCategory.Core, "Province prefab'ları bulunamadı!");
            return;
        }
        ...
        GameLog.Log(GameLogCategory.Core, $"✓ {loadedCount} province yüklendi (collider'lar oluşturuldu)!");
    ...
    void DeleteAllProvinces()
    {
        ...
        if (mapGen != null)
        {
            ...
            GameLog.Log(GameLogCategory.Core, $"✓ {provinces.Length} province silindi!");
        }
    }
