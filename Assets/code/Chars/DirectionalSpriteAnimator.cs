...
public class DirectionalSpriteAnimator : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Replace the prefab's baked placeholder sprite with the proper idle sprite
        // immediately, so a stale sprite never shows before the character first moves.
    ...
    private void Update()
    {
        ...
        if (frameTimer >= frameInterval)
        {
            ...
            if (spriteRenderer != null && currentFrames[currentFrame] != null)
            {
                ...
                {
                    GameLog.Log(GameLogCategory.Core, $"[Animator] Frame {currentFrame}: {currentFrames[currentFrame].name}");
                }
            }
        }
    }
    ...
    private void SetDirectionData(Direction dir)
    {
        ...
        if (logAnimationChanges)
        {
            GameLog.Log(GameLogCategory.Core, $"[Animator] Direction: {dir}");
        }
    }
