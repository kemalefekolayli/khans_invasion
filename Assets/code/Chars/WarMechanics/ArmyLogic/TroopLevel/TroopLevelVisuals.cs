...
public class TroopLevelVisuals : MonoBehaviour
{
    ...
    private void EnsureProgressBarExists()
    {
        ...
        GameLog.Log(GameLogCategory.Core, "[TroopLevelVisuals] Created progress bar child object");
    }
    ...
    private void UpdateTroopSprite(TroopLevelData data)
    {
        ...
        if (data.IsMaxLevel)
        {
            if (IsValidIndex(goldenSprites, colorIndex))
            {
                ...
                GameLog.Log(GameLogCategory.Core, $"[TroopLevelVisuals] Set GOLDEN sprite ({data.colorVariant})");
            }
            else
            {
                GameLog.Warning(GameLogCategory.Core, $"[TroopLevelVisuals] Golden sprite missing for {data.colorVariant}");
            }
        }
        else
        {
            ...
            {
                GameLog.Warning(GameLogCategory.Core, $"[TroopLevelVisuals] Regular sprite missing for {data.colorVariant}");
            }
        }
    }
    ...
    private void UpdateProgressBar(TroopLevelData data)
    {
        ...
        {
            GameLog.Warning(GameLogCategory.Core, $"[TroopLevelVisuals] Progress sprite missing for level {data.currentLevel}, color {data.colorVariant}");
        }
    }
    ...
    {
        GameLog.Log(GameLogCategory.Core, $"Regular sprites: {CountAssigned(regularSprites)}/6");
        GameLog.Log(GameLogCategory.Core, $"Golden sprites: {CountAssigned(goldenSprites)}/6");
        GameLog.Log(GameLogCategory.Core, $"Progress L1: {CountAssigned(progressLevel1)}/6");
        GameLog.Log(GameLogCategory.Core, $"Progress L2: {CountAssigned(progressLevel2)}/6");
        GameLog.Log(GameLogCategory.Core, $"Progress L3: {CountAssigned(progressLevel3)}/6");
        GameLog.Log(GameLogCategory.Core, $"Progress L4: {CountAssigned(progressLevel4)}/6");
    }
