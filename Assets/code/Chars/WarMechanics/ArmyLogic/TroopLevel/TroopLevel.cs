...
public class TroopLevel : MonoBehaviour
{
    ...
    public void GainXP(float amount)
    {
        if (data.IsMaxLevel)
        {
            GameLog.Log(GameLogCategory.Core, $"[TroopLevel] {army?.Data.armyName} is max level, XP ignored");
            return;
        }
    ...
    public void ForceLevelUp()
    {
        if (data.IsMaxLevel)
        {
            GameLog.Log(GameLogCategory.Core, "[TroopLevel] Already at max level!");
            return;
        }
    ...
    private void OnLevelUp(int fromLevel, int toLevel)
    {
        ...
        GameLog.Log(GameLogCategory.Core, $"✓ [{armyName}] LEVEL UP: {fromLevel} -> {toLevel}!");
        ...
        {
            GameLog.Log(GameLogCategory.Core, $"★ [{armyName}] reached MAX LEVEL - GOLDEN ELITE!");
        }
    }
    ...
    {
        GameLog.Log(GameLogCategory.Core, $"[TroopLevel] Level: {data.currentLevel}/{TroopLevelData.MAX_LEVEL}, " +
