...
public class DirectionalSprite : MonoBehaviour
{
    ...
    private void UpdateSprite()
    {
        ...
        if (index >= 0 && index < directionSprites.Length)
        {
            ...
            {
                GameLog.Warning(GameLogCategory.Core, $"[DirectionalSprite] No sprite assigned for direction: {currentDirection}");
            }
        }
    }
    ...
    private System.Collections.IEnumerator CycleDirectionsCoroutine()
    {
        for (int i = 0; i < 8; i++)
        {
            ...
            GameLog.Log(GameLogCategory.Core, $"Direction: {(Direction)i}");
