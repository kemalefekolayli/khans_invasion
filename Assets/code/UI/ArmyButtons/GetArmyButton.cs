...
public class GetArmyButton : MonoBehaviour
{
    ...
    public void OnGetArmyButtonClicked()
    {
        ...
        {
            GameLog.Warning(GameLogCategory.Core, $"[GetArmyButton] Province {province.provinceName} has insufficient population ({province.provinceCurrentPop:F0} < {minPopulationRequired})");
        ...
        if (logActions)
        {
            GameLog.Log(GameLogCategory.Core, $"[GetArmyButton] Conscripted {actualRecruit:F0} troops from {province.provinceName}. Pop: {province.provinceCurrentPop:F0}");
        }
    ...
    private void AddTroopsToExistingArmy(General general, SelectableGeneral selectable, float amount)
    {
        ...
        if (logActions)
        {
            GameLog.Log(GameLogCategory.Core, $"[GetArmyButton] {selectable.DisplayName}'s army: {currentSize}/{maxSize} (Space: {availableSpace})");
        }
        ...
        if (availableSpace >= amount)
        {
            ...
            if (logActions)
            {
                GameLog.Log(GameLogCategory.Core, $"✓ [GetArmyButton] Added {amount} troops to {selectable.DisplayName}'s army. New size: {existingArmy.ArmySize}");
            }
        }
        else if (availableSpace > 0)
        {
            ...
            if (logActions)
            {
                GameLog.Log(GameLogCategory.Core, $"[GetArmyButton] {selectable.DisplayName}'s army maxed at {maxSize}. Distributing {overflow} overflow troops...");
            }
        ...
        else
        {
            ...
            if (logActions)
            {
                GameLog.Log(GameLogCategory.Core, $"[GetArmyButton] {selectable.DisplayName}'s army is FULL. Distributing {amount} troops to reserves...");
            }
    ...
    private void CreateNewArmyForGeneral(General general, SelectableGeneral selectable, float amount)
    {
        ...
        if (army != null)
        {
            ...
            if (logActions)
            {
                GameLog.Log(GameLogCategory.Core, $"✓ [GetArmyButton] Created new army for {selectable.DisplayName} (Size: {amount}, Max: {defaultMaxArmySize})");
            }
    ...
    private void DistributeTroopsToIdleArmies(SelectableGeneral selectable, float troopCount)
    {
        ...
        {
            GameLog.Warning(GameLogCategory.Core, "[GetArmyButton] ArmyManager not found! Creating new army directly.");
        ...
        foreach (Army idleArmy in idleArmiesWithSpace)
        {
            ...
            if (toAdd > 0)
            {
                ...
                if (logActions)
                {
                    GameLog.Log(GameLogCategory.Core, $"  → Added {toAdd} troops to existing reserve army. Now: {idleArmy.ArmySize}/{idleArmy.Data.maxSize}");
                }
            }
        }
        ...
        else
        {
            if (logActions)
            {
                GameLog.Log(GameLogCategory.Core, $"✓ [GetArmyButton] All troops consolidated into existing armies!");
            }
        }
    }
    ...
    private void CreateStandaloneArmy(SelectableGeneral selectable, float size)
    {
        ...
        if (army != null)
        {
            ...
            if (logActions)
            {
                GameLog.Log(GameLogCategory.Core, $"✓ [GetArmyButton] Created new reserve army near {selectable.DisplayName} (Size: {size}, Max: {defaultMaxArmySize})");
            }
