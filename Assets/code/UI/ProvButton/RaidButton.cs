...
public class RaidButton : MonoBehaviour
{
    ...
    private void OnRaidButtonClicked()
    {
        ...
        {
            GameLog.Warning(GameLogCategory.Core, "[RaidButton] No province selected!");
            return;
        }
        
        if (RaidManager.Instance == null)
        {
            GameLog.Error(GameLogCategory.Core, "[RaidButton] RaidManager not found!");
            return;
        }
        ...
        if (raider == null)
        {
            GameLog.Warning(GameLogCategory.Core, "[RaidButton] No raider found!");
            return;
        }
        ...
        if (lootGained > 0)
        {
            GameLog.Log(GameLogCategory.Core, $"[RaidButton] Raid successful! Gained {lootGained:F0} loot.");
    ...
    private void PlayRaidAnimation()
    {
        ...
        GameLog.Log(GameLogCategory.Core, "[RaidButton] Playing raid animation (placeholder)");
    }
