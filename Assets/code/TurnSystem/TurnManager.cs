...
public class TurnManager : MonoBehaviour
{
    ...
    public void EndPlayerTurn()
    {
        ...
        {
            GameLog.Warning(GameLogCategory.Core, $"[TurnManager] Cannot end turn - not player's turn! (Phase: {currentPhase})");
            return;
        }
    ...
    private void ProcessAITurns()
    {
        ...
        if (logTurnEvents)
        {
            GameLog.Log(GameLogCategory.Core, "[TurnManager] Processing AI turns...");
        }
    ...
    private void ProcessEndTurnCalculations()
    {
        ...
        // Process all registered turn processors, lowest priority first
        turnProcessors.Sort((a, b) => a.ProcessingPriority.CompareTo(b.ProcessingPriority));
        foreach (var processor in turnProcessors)
        {
            ...
            {
                GameLog.Error(GameLogCategory.Core, $"[TurnManager] Error in turn processor: {e.Message}");
            }
        }
    ...
    public void SetAIEnabled(bool enabled)
    {
        ...
        GameLog.Log(GameLogCategory.Core, $"[TurnManager] AI turns {(enabled ? "ENABLED" : "DISABLED")}");
    }
    ...
    {
        GameLog.Log(GameLogCategory.Core, $"=== Turn Manager State ===");
        GameLog.Log(GameLogCategory.Core, $"Turn: {currentTurn}");
        GameLog.Log(GameLogCategory.Core, $"Phase: {currentPhase}");
        GameLog.Log(GameLogCategory.Core, $"Registered Processors: {turnProcessors.Count}");
    }
