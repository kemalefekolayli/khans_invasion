...
public class IncomeProcessor : MonoBehaviour, ITurnProcessor
{
    ...
    public void ProcessTurnEnd(int turnNumber)
    {
        if (playerNation == null)
        {
            ...
            if (playerNation == null)
            {
                GameLog.Warning(GameLogCategory.Core, "[IncomeProcessor] PlayerNation not found!");
                return;
            }
        }
        ...
        if (logIncome)
        {
            GameLog.Log(GameLogCategory.Core, $"[IncomeProcessor] Turn {turnNumber} Income:");
            GameLog.Log(GameLogCategory.Core, $"  Tax:   +{taxIncome:F0} (×{taxMultiplier})");
            GameLog.Log(GameLogCategory.Core, $"  Trade: +{tradeIncome:F0} (×{tradeMultiplier})");
            GameLog.Log(GameLogCategory.Core, $"  Total: +{totalIncome:F0}");
            GameLog.Log(GameLogCategory.Core, $"  Gold:  {previousGold:F0} → {playerNation.nationMoney:F0}");
        }
    ...
    public void SetTaxMultiplier(float multiplier)
    {
        ...
        GameLog.Log(GameLogCategory.Core, $"[IncomeProcessor] Tax multiplier set to: {multiplier}");
    }
    ...
    public void SetTradeMultiplier(float multiplier)
    {
        ...
        GameLog.Log(GameLogCategory.Core, $"[IncomeProcessor] Trade multiplier set to: {multiplier}");
    }
