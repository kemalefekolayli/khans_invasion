...
public class PopulationProcessor : MonoBehaviour, ITurnProcessor
{
    ...
    public void ProcessTurnEnd(int turnNumber)
    {
        ...
        {
            GameLog.Log(GameLogCategory.Core, $"[PopulationProcessor] Turn {turnNumber}: {provincesGrown} provinces grew by total {totalGrowth:F0} population");
        }
    }
    ...
    private float GrowPopulation(ProvinceModel province)
    {
        ...
        {
            GameLog.Log(GameLogCategory.Core, $"[PopulationProcessor] {province.provinceName}: {currentPop:F0} → {newPop:F0} (+{actualGrowth:F0}, {growthPercent:P1})");
        }
