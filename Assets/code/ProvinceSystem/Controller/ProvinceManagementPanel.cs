...
public class ProvinceManagementPanel : MonoBehaviour
{
    ...
    {
        GameLog.Log(GameLogCategory.Core, $"[MgmtPanel] OnManagementOpened: {province?.provinceName}, isVisible={isVisible}");
    ...
    {
        GameLog.Log(GameLogCategory.Core, $"[MgmtPanel] OnPanelClosed called, isVisible={isVisible}");
        HidePanel();
    }

    private void OnCityCenterExit(CityCenter cityCenter)
    {
        GameLog.Log(GameLogCategory.Core, $"[MgmtPanel] OnCityCenterExit: {cityCenter?.Province?.provinceName}, isVisible={isVisible}");
        if (isVisible)
        {
            GameLog.Log(GameLogCategory.Core, $"[MgmtPanel] Panel is visible -> ClosePanel()");
            ClosePanel();
        }
    }
    ...
    {
        GameLog.Log(GameLogCategory.Core, $"[MgmtPanel] ShowPanel -> isVisible=true");
        isVisible = true;
        if (panelRoot != null)
    ...
    {
        GameLog.Log(GameLogCategory.Core, $"[MgmtPanel] HidePanel -> isVisible=false");
        isVisible = false;
    ...
    private void TryBuild(string buildingType)
    {
        ...
        {
            GameLog.Warning(GameLogCategory.Core, "Cannot build - no province or player");
            return;
        }
    ...
    {
        GameLog.Log(GameLogCategory.Core, $"[MgmtPanel] ClosePanel -> firing ProvincePanelClosed event");
