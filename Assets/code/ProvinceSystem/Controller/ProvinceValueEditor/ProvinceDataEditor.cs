...
public class ProvinceDataEditor : EditorWindow
{
    ...
    private bool localHasUnsavedChanges = false;
    ...
    private void OnEditorUpdate()
    {
        ...
        if (autoSave && localHasUnsavedChanges && EditorApplication.timeSinceStartup - lastChangeTime > AUTO_SAVE_DELAY)
        {
            SaveAllToJson(silent: true);
    ...
    private void RefreshData()
    {
        ...
        GameLog.Log(GameLogCategory.Core, $"[ProvinceDataEditor] Refreshed: {sceneProvinces.Count} provinces, {provinceDataById.Count} data entries");
    }
    ...
    private void SelectProvince(ProvinceModel province)
    {
        ...
        if (selectedData != null && localHasUnsavedChanges)
        {
            provinceDataById[selectedData.provinceId] = selectedData;
        }
    ...
    private void ApplyToSceneObject()
    {
        ...
        GameLog.Log(GameLogCategory.Core, $"[ProvinceDataEditor] Applied data to scene object: {selectedProvince.provinceName}");
    }
    ...
    private void GenerateDefaultDataForAll()
    {
        ...
        GameLog.Log(GameLogCategory.Core, $"[ProvinceDataEditor] Generated default data for {count} provinces");
        SaveAllToJson(silent: false);
    }
    ...
    private void ExportCurrentValuesToJson()
    {
        ...
        SaveAllToJson(silent: false);
        GameLog.Log(GameLogCategory.Core, $"[ProvinceDataEditor] Exported {provinceDataById.Count} provinces to JSON");
    }
    ...
    private void SaveAllToJson(bool silent = false)
    {
        ...
        if (!silent)
        {
            GameLog.Log(GameLogCategory.Core, $"✓ Saved {wrapper.provinces.Length} provinces to {path}");
    ...
    private void LoadDataFromJson()
    {
        ...
        if (!File.Exists(path))
        {
            GameLog.Log(GameLogCategory.Core, $"[ProvinceDataEditor] No existing data file at {path}");
            return;
        }
        ...
        {
            GameLog.Error(GameLogCategory.Core, "[ProvinceDataEditor] Failed to parse province_data.json");
            return;
        }
        ...
        GameLog.Log(GameLogCategory.Core, $"✓ Loaded {wrapper.provinces.Length} provinces from JSON (v{wrapper.version}, modified: {wrapper.lastModified})");
    }
    ...
    private void ApplyDataToScene()
    {
        ...
        GameLog.Log(GameLogCategory.Core, $"✓ Applied data to {appliedCount} scene provinces");
