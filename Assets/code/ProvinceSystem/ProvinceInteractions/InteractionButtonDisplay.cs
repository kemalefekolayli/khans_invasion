...
public class InteractionButtonDisplay : MonoBehaviour
{
    ...
    {
        GameLog.Log(GameLogCategory.Core, $"[ButtonDisplay] OnPanelOpened -> isPanelOpen=true (was {isPanelOpen})");
        isPanelOpen = true;
    }

    private void OnPanelClosed()
    {
        GameLog.Log(GameLogCategory.Core, $"[ButtonDisplay] OnPanelClosed -> isPanelOpen=false (was {isPanelOpen})");
        isPanelOpen = false;
    }
    ...
    private void Update()
    {
        ...
        if (selectionManager != null && selectionManager.SelectedGeneral != null)
        {
            ...
            {
                GameLog.Log(GameLogCategory.Core, "[ButtonDisplay] Safety reset: general left city center, clearing isPanelOpen");
                isPanelOpen = false;
            }
        ...
        {
            GameLog.Log(GameLogCategory.Core, $"[ButtonDisplay] shouldShow changed: {_lastShouldShow} -> {shouldShow} (CC={currentCityCenter?.Province?.provinceName}, isPanelOpen={isPanelOpen})");
    ...
    public void DebugForceShow()
    {
        ...
        GameLog.Log(GameLogCategory.Core, "[ButtonDisplay] FORCED VISIBLE");
    }
}