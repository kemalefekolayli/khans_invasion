...
public class ProvinceRaidOverlay : MonoBehaviour
{
    ...
    private void OnProvinceOwnerChanged(ProvinceModel changedProvince, NationModel oldOwner, NationModel newOwner)
    {
        if (changedProvince == province && provinceRenderer != null)
        {
            ...
            GameLog.Log(GameLogCategory.Core, $"[ProvinceRaidOverlay] Updated original color for {province?.provinceName} after conquest");
        }
    }
    
    private void OnTurnStart(int turnNumber)
    {
        // Only manage tint for provinces we've actually raided. Otherwise RemoveTint()
        // would reset every un-raided province to its (bright) captured originalColor
        // every turn, overwriting the fog-of-war color and permanently lifting the fog.
        if (!IsRaided) return;
    ...
    public void PlayRaidAnimation()
    {
        ...
        GameLog.Log(GameLogCategory.Core, $"[ProvinceRaidOverlay] Playing raid animation for {province?.provinceName}");
    }
}
