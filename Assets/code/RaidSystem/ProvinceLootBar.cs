...
public class ProvinceLootBar : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (fillImage != null)
        {
            if (fillImage.type != Image.Type.Filled)
            {
                ...
                GameLog.Log(GameLogCategory.Core, $"[ProvinceLootBar] Set Image type to Filled");
            }
            ...
            if (fillImage.fillMethod != Image.FillMethod.Horizontal)
            {
                ...
                GameLog.Log(GameLogCategory.Core, $"[ProvinceLootBar] Set Fill Method to Horizontal (Left origin)");
            }
        }
    ...
    private void OnProvinceRaided(ProvinceModel province, General raider, float lootAmount)
    {
        ...
        {
            GameLog.Log(GameLogCategory.Core, $"[ProvinceLootBar] {province.provinceName} was raided, updating display");
            RefreshLootDisplay();
        }
    }
    ...
    public void RefreshLootDisplay()
    {
        ...
        {
            GameLog.Warning(GameLogCategory.Core, $"[ProvinceLootBar] No province set!");
            targetFill = 0f;
            if (!animateFill)
            {
        ...
        {
            GameLog.Warning(GameLogCategory.Core, $"[ProvinceLootBar] RaidManager not found!");
            targetFill = 1f;
            if (!animateFill)
            {
                currentFill = targetFill;
