...
public class RaidSmokeController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            GameLog.Warning(GameLogCategory.Core, "[RaidSmokeController] No smoke sprites assigned!");
        }
        else
        {
            GameLog.Log(GameLogCategory.Core, $"[RaidSmokeController] Initialized with {smokeSprites.Length} sprites, pool size: {poolSize}");
        }
    }
    ...
    private void InitializePool()
    {
        ...
        if (logEvents)
        {
            GameLog.Log(GameLogCategory.Core, $"[RaidSmokeController] Created pool of {poolSize} smoke objects");
        }
    }
    ...
    private void OnProvinceRaided(ProvinceModel province, General raider, float lootAmount)
    {
        ...
        if (logEvents)
        {
            GameLog.Log(GameLogCategory.Core, $"[RaidSmokeController] 💨 Raid at {province.provinceName}: {burstCount} burst, then continuous for {smokeDurationTurns} turns");
        }
    }
    ...
    private void OnTurnEnded(int turnNumber)
    {
        ...
        foreach (var kvp in activeSources)
        {
            ...
            if (source.TurnsRemaining <= 0)
            {
                ...
                if (logEvents)
                {
                    GameLog.Log(GameLogCategory.Core, $"[RaidSmokeController] Smoke ended for {source.Province.provinceName}");
                }
            }
            else if (logEvents)
            {
                GameLog.Log(GameLogCategory.Core, $"[RaidSmokeController] {source.Province.provinceName}: {source.TurnsRemaining} turns of smoke remaining");
            }
        }
