...
public class QuestPanelButton : MonoBehaviour
{
    ...
    private void InstantiatePrefab()
    {
        ...
        if (prefab == null)
        {
            GameLog.Error(GameLogCategory.Core, "[QuestPanelButton] Cannot find QuestPanelCanvas prefab! Either assign it in Inspector or put it in Resources folder.");
            return;
        }
        ...
        if (cachedController == null)
        {
            GameLog.Error(GameLogCategory.Core, "[QuestPanelButton] QuestPanelController component not found on prefab!");
        }
        else
        {
            GameLog.Log(GameLogCategory.Core, "[QuestPanelButton] QuestPanelCanvas instantiated successfully");
        }
    }
}
