...
public class SiegeSmokeController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            GameLog.Warning(GameLogCategory.Core, "[SiegeSmokeController] No smoke sprites assigned!");
        }
        else
        {
            GameLog.Log(GameLogCategory.Core, $"[SiegeSmokeController] Initialized with {smokeSprites.Length} sprites, pool size: {poolSize}");
        }
    }
    ...
    private void InitializePool()
    {
        ...
        if (logEvents)
        {
            GameLog.Log(GameLogCategory.Core, $"[SiegeSmokeController] Created pool of {poolSize} smoke objects");
        }
    }
    ...
    private void OnProvinceSieged(ProvinceModel province, General attacker, float defenseStrength)
    {
        ...
        if (!activeSiegeSources.ContainsKey(province.provinceId))
        {
            ...
            if (logEvents)
            {
                GameLog.Log(GameLogCategory.Core, $"[SiegeSmokeController] 💨 Fortress siege at {province.provinceName}: {smokesPerSpawn} smokes per spawn");
            }
        }
    }
    ...
    private void OnProvinceConquered(ProvinceModel province, NationModel oldOwner, NationModel newOwner)
    {
        ...
        if (logEvents)
        {
            GameLog.Log(GameLogCategory.Core, $"[SiegeSmokeController] 💨 Province conquered: {province.provinceName}, burst: {burstCount}, continues for {conquestSmokeTurns} turns");
        }
    }
    ...
    private void OnSiegeCancelled(ProvinceModel province)
    {
        ...
        if (activeSiegeSources.Remove(province.provinceId))
        {
            if (logEvents)
            {
                GameLog.Log(GameLogCategory.Core, $"[SiegeSmokeController] Siege cancelled at {province.provinceName}, stopping smoke");
            }
        }
    }
    ...
    private void OnTurnEnded(int turnNumber)
    {
        ...
        foreach (var kvp in conquestSources)
        {
            ...
            if (source.TurnsRemaining <= 0)
            {
                ...
                if (logEvents)
                {
                    GameLog.Log(GameLogCategory.Core, $"[SiegeSmokeController] Conquest smoke ended for {source.Province.provinceName}");
                }
            }
        }
