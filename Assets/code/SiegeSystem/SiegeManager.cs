...
public class SiegeManager : MonoBehaviour, ITurnProcessor
{
    ...
    public SiegeResult ExecuteSiege(ProvinceModel province, General attacker)
    {
        ...
        if (canSiege != SiegeResult.Success)
        {
            ...
            if (logSiegeEvents)
            {
                GameLog.Log(GameLogCategory.Core, $"[SiegeManager] Siege FAILED on {province?.provinceName}: {canSiege}");
            }
        ...
        if (logSiegeEvents)
        {
            GameLog.Log(GameLogCategory.Core, $"[SiegeManager] ═══ SIEGE STARTED ═══");
            GameLog.Log(GameLogCategory.Core, $"  Attacker: {attacker.GeneralName} ({armySize:F0} troops)");
            GameLog.Log(GameLogCategory.Core, $"  Province: {province.provinceName}");
            GameLog.Log(GameLogCategory.Core, $"  Defense: {defenseStrength:F0}");
            GameLog.Log(GameLogCategory.Core, $"  Fortress: {hasFortress}");
            GameLog.Log(GameLogCategory.Core, $"  Turns Required: {state.turnsRemaining}");
            GameLog.Log(GameLogCategory.Core, $"  Initial Casualties: {casualties} ({casualtyPercent:P1})");
            GameLog.Log(GameLogCategory.Core, $"  Army After: {army.ArmySize:F0}");
        }
    ...
    private void ApplyFailedSiegeCasualties(ProvinceModel province, General attacker)
    {
        ...
        if (logSiegeEvents)
        {
            GameLog.Log(GameLogCategory.Core, $"[SiegeManager] ═══ SIEGE FAILED - DEVASTATING LOSSES ═══");
            GameLog.Log(GameLogCategory.Core, $"  Attacker: {attacker.GeneralName}");
            GameLog.Log(GameLogCategory.Core, $"  Province Defense: {defenseStrength:F0}");
            GameLog.Log(GameLogCategory.Core, $"  Casualties: {casualties} ({casualtyPercent:P1})");
            GameLog.Log(GameLogCategory.Core, $"  Army Remaining: {army.ArmySize:F0}");
        }
    }
    ...
    public void ProcessTurnEnd(int turnNumber)
    {
        ...
        foreach (var kvp in activeSieges)
        {
            ...
            if (state.isFortress && presentGenerals.Count == 0)
            {
                ...
                if (logSiegeEvents)
                {
                    GameLog.Log(GameLogCategory.Core, $"[SiegeManager] Siege of {province.provinceName} CANCELLED - army abandoned fortress siege!");
                }
                continue;
            }
            ...
            if (state.isFortress && state.turnsRemaining > 1 && presentGenerals.Count > 0)
            {
                ...
                foreach (General general in presentGenerals)
                {
                    if (general.HasArmy)
                    {
                        ...
                        {
                            GameLog.Log(GameLogCategory.Core, $"[SiegeManager] Turn {turnNumber} siege casualties: {general.GeneralName} lost {casualties} ({casualtyPercent:P1})");
                        }
            ...
            {
                GameLog.Log(GameLogCategory.Core, $"[SiegeManager] Siege of {province.provinceName}: {state.turnsRemaining} turns remaining");
            }
        }
    ...
    private void OnPlayerTurnStart()
    {
        ...
        if (logSiegeEvents)
        {
            GameLog.Log(GameLogCategory.Core, $"[SiegeManager] Processing {toConquer.Count} completed sieges...");
        }
        
        PlayerNation player = PlayerNation.Instance;
        if (player == null || player.currentNation == null)
        {
            GameLog.Error(GameLogCategory.Core, "[SiegeManager] PlayerNation not found! Cannot process conquests.");
            return;
        }
        ...
        foreach (ProvinceModel province in toConquer)
        {
            ...
            if (state.isFortress)
            {
                ...
                else
                {
                    ...
                    if (logSiegeEvents)
                    {
                        GameLog.Log(GameLogCategory.Core, $"[SiegeManager] Fortress siege of {province.provinceName} failed - no army at completion!");
                    }
                    GameEvents.SiegeCancelled(province);
                }
            }
    ...
    private void ConquerProvince(ProvinceModel province, NationModel newOwner)
    {
        ...
        if (oldOwner != null && oldOwner.provinceList.Contains(province))
        {
            ...
            if (logSiegeEvents)
            {
                GameLog.Log(GameLogCategory.Core, $"[SiegeManager] Removed {province.provinceName} from {oldOwner.nationName}");
            }
        }
        ...
        if (logSiegeEvents)
        {
            GameLog.Log(GameLogCategory.Core, $"[SiegeManager] ✓ {province.provinceName} conquered by {newOwner.nationName}!");
        }
