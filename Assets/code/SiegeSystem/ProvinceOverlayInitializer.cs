...
public class ProvinceOverlayInitializer : MonoBehaviour
{
    ...
    private void TryFallbackInitialization()
    {
        if (!hasInitialized)
        {
            GameLog.Warning(GameLogCategory.Core, "[ProvinceOverlayInitializer] Event didn't fire, using fallback initialization");
    ...
    private void InitializeAllProvinces()
    {
        if (hasInitialized)
        {
            GameLog.Log(GameLogCategory.Core, "[ProvinceOverlayInitializer] Already initialized, skipping");
            return;
        }
        ...
        {
            GameLog.Warning(GameLogCategory.Core, "[ProvinceOverlayInitializer] No provinces found!");
            return;
        }
        ...
        if (logInitialization)
        {
            ...
            if (addSiegeOverlay)
                GameLog.Log(GameLogCategory.Core, $"  - Added SiegeOverlayController to {siegeAdded} provinces");
            if (addRaidOverlay)
                GameLog.Log(GameLogCategory.Core, $"  - Added ProvinceRaidOverlay to {raidAdded} provinces");
        }
    }
}
