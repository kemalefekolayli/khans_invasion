...
public class SiegeSystemInitializer : MonoBehaviour
{
    ...
    private void TryFallbackInitialization()
    {
        if (!hasInitialized)
        {
            GameLog.Warning(GameLogCategory.Core, "[SiegeSystemInitializer] Event didn't fire, using fallback initialization");
    ...
    private void InitializeAllProvinces()
    {
        if (hasInitialized)
        {
            GameLog.Log(GameLogCategory.Core, "[SiegeSystemInitializer] Already initialized, skipping");
            return;
        }
        ...
        {
            GameLog.Warning(GameLogCategory.Core, "[SiegeSystemInitializer] No provinces found!");
            return;
        }
